...
router.use((req, res, next) => {
    if (req.path == '/login' || req.path == '/send_authcode') {
        next();
    }
    else {
        if (req.cookies.jwt) {
...
router.get('/unchecked_users', catchAsyncErrors(async (req, res, next) => {
    ...
    if (userQx.indexOf('7') > -1) {
        ...
        let userApplication1 = await orm.ApplicationRecord.findAll({ where: {CSR: payload.jsIndex}, attributes: ['XMDDM'] });
        let xmddms1 = userApplication1.map(app => app.XMDDM);
        let uncheckedUsers1 = await orm.CustomerInfo.findAll({ where: { ZT: '1', XMDDM: { [orm.Sequelize.Op.in]: xmddms1 } } });
       
        let userApplication2 = await orm.ApplicationRecord.findAll({ where: {FSR: payload.jsIndex}, attributes: ['XMDDM'] });
        let xmddms2 = userApplication2.map(app => app.XMDDM);
        let uncheckedUsers2 = await orm.CustomerInfo.findAll({ where: { ZT: '2', XMDDM: { [orm.Sequelize.Op.in]: xmddms2 } } });
        
        let userApplication3 = await orm.ApplicationRecord.findAll({ where: {SQSHR: payload.jsIndex}, attributes: ['XMDDM'] });
        let xmddms3 = userApplication3.map(app => app.XMDDM);
        let uncheckedUsers3 = await orm.CustomerInfo.findAll({ where: { ZT: '3', XMDDM: { [orm.Sequelize.Op.in]: xmddms3 } } });
...
router.get('/unchecked_applications', catchAsyncErrors(async (req, res, next) => {
    ...
    if (userQx.indexOf('7') > -1) {
        ...
        // let application = await orm.ApplicationRecord.findAll({
        //     include: [{
        ...
        // let allApplications2 = [];
        // for (let app of userApplication2) {
        ...
        //         }
        //     });
        //     allApplications2 = allApplications2.concat(responseObjects);
        // }
        ...
        // let allApplications3 = [];
        // for (let app of userApplication3) {
        //     let uncheckedApplications = await orm.ApplicationInfo.findAll({
        //         where: {
        //             BLZT: '3',
        //             XMDDM: app.XMDDM,
        ...
        //         }
        //     });
        //     allApplications3 = allApplications3.concat(responseObjects);
        // }

        // let userApplication4 = await orm.ApplicationRecord.findAll({
        //     where:
        //         {
        ...
        // let allApplications4 = [];
        // for (let app of userApplication4) {
        //     let uncheckedApplications = await orm.ApplicationInfo.findAll({
        //         where: {
        //             BLZT: '4',
        //             XMDDM: app.XMDDM,
        ...
        //         }
        //     });
        //     allApplications4 = allApplications4.concat(responseObjects);
        // }

        // let allApplications = allApplications2.concat(allApplications3).concat(allApplications4);
        ...
        // let userApplication = userApplication2.concat(userApplication3).concat(userApplication4);
        //------------------------------------------------------------------------------------------//
...
router.post('/checked_applications', catchAsyncErrors(async (req, res) => {
    ...
    let blzt = sqInfo[0].BLZT;
    let lxdhObj = await orm.CustomerInfo.findAll({ where: { INDEX: sqInfo[0].YHRZXX_INDEX }, attributes: ['LXDH'] });
...
router.post('/role/jscy_delete_tip', catchAsyncErrors(async (req, res) => {
    ...
    let results=[];
    for(let i= 0;i<reqParams.index.length;i++){
        relatedItems = await orm.ApplicationRecord.findAll({ where: { [orm.Sequelize.Op.or]: [{ csr: reqParams.index[i] }, { fsr: reqParams.index[i] }, { sqshr: reqParams.index[i] }]   } });
    ...
    if (results.length === 0) {
        return res.json({related:'0'});  
    } else {
        return res.json({related:'1'});     
    }
...
router.get('/get_lrr', catchAsyncErrors(async (req, res) => {
    ...
    let lrr = await orm.User.findOne({ where: { LXDH: payload.phone} });
    return res.json({code:'0000',lrr_index:lrr.INDEX, lrr: lrr.JSCY})
}));
...
router.get('/get_ssqy', catchAsyncErrors(async (req, res) => {
    ...
    let ssqydms = await orm.ApplicationRecord.findAll({group:['XMDDM'],attributes:[['XMDDM','dm'],['XMDMC','mc']]});
